...
function analyzePrices(items, costPrice) {
  ...
  if (costPrice != null && !Number.isNaN(costPrice)) {
    // `suggested` is already a total landed price, so if you list with free
    // shipping (the common, competitive approach), eBay's ~13% fee applies
    // to that whole amount — which this already reflects correctly.
    estimatedMargin = +(
      suggested -
      costPrice -
    ...
    const sellerCount = landedPrices.length;
...
module.exports = { analyzePrices, detectPackQuantity };
